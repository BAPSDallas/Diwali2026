...
(async () => {
  ...
  await page.getByRole('button',{name:'Clear optional selections'}).click();
  assert.equal(await page.locator('input:checked').count(),0);
  await page.getByRole('button',{name:'Select all optional events'}).click();
  assert.equal(await page.locator('input:checked').count(),3);
  await page.getByRole('button',{name:'Clear optional selections'}).click();
  await page.locator('label.event-card').first().locator('h3').click();
  assert(await page.locator('input[value=kdc]').isChecked());
  ...
  await browser.close();
  console.log('PASS: all 8 browser downloads, optional card toggles, select/clear all, no overflow at 320/390/768/1280px, no JS errors.');
