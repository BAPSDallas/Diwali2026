...
for (let mask = 0; mask < 8; mask++) {
  test(`calendar selection combination ${mask}`, () => {
    const ids = optional.filter((_, i) => mask & (1 << i));
...
test('all five events have exact requested times, titles, locations and description', () => {
  const text = unfold(buildCalendar(optional));
  ...
  assert.equal(fs.readFileSync('Diwali_Events_Dallas_Frisco_2026.ics', 'utf8'), buildCalendar(optional));
});
