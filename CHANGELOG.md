...
- Show only the action that works on the visitor's device, with the other behind a link: Android sees Google Calendar, everything else sees the .ics file. iPad detection uses `maxTouchPoints` because iPadOS reports itself as a Mac.
