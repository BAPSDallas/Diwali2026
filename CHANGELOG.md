...
- Offer Apple Calendar and Google Calendar side by side with their brand marks, with the device deciding only which one is filled in. Detection moves emphasis rather than hiding a path, since neither path works on the other's platform. iPad detection uses `maxTouchPoints` because iPadOS reports itself as a Mac.
